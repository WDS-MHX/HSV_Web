...
export default function XayDungHoi() {
  const router = useRouter()
  const itemsPerPage = 4

  const [value, setValue] = useState<string>('')
  const { data, isFetchingNextPage, hasNextPage, fetchNextPage } = useInfiniteQuery({
    queryKey: queryKeys.allPosts.gen(POST_CATEGORY.XAY_DUNG_HOI),
    queryFn: ({ pageParam }) =>
      postApi.getAllPostsByCategory(pageParam, itemsPerPage, POST_CATEGORY.XAY_DUNG_HOI),
    initialPageParam: 1,
